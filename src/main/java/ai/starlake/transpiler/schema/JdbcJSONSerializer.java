...
public class JdbcJSONSerializer {
  ...
  protected static JSONObject toJson(JdbcCatalog catalog) {
    ...
    catalogObject.put("name", catalog.getTableCatalog());
    catalogObject.put("separator", catalog.getCatalogSeparator());
  ...
  protected static JSONObject toJson(JdbcColumn column) {
    ...
    columnObject.put("name", column.getColumnName());
    columnObject.put("type", column.getTypeName());
    columnObject.put("typeID", column.getDataType());
    columnObject.put("size", column.getColumnSize());
    if (column.getDecimalDigits() != null) {
      columnObject.put("decimalDigits", column.getDecimalDigits());
    }
  ...
  protected static JdbcColumn fromJsonColumn(JSONObject json) {
    ...
    column.setColumnName(json.getString("name"));
    column.setTypeName(json.getString("type"));
    column.setDataType(json.getInt("typeID"));
    column.setColumnSize(json.getInt("size"));
    column.setIsNullable(json.getBoolean("isNullable"));
    if (json.has("decimalDigits")) {
      column.setDecimalDigits(json.getInt("decimalDigits"));
    }
