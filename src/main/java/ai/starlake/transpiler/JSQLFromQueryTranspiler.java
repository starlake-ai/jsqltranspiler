...
public class JSQLFromQueryTranspiler implements FromQueryVisitor<PlainSelect, PlainSelect>, PipeOperatorVisitor<PlainSelect, PlainSelect> {
    ...
    public PlainSelect visit(AggregatePipeOperator aggregatePipeOperator, PlainSelect plainSelect) {
        if (plainSelect.getSelectItems()==null || plainSelect.getSelectItems().isEmpty()) {
            ...
            plainSelect = plainSelect.withFromItem(plainSelect).withSelectItems(aggregatePipeOperator.getSelectItems());
        }

        if (aggregatePipeOperator.getGroupItems()!=null && !aggregatePipeOperator.getGroupItems().isEmpty()) {
    ...
    public PlainSelect visit(AsPipeOperator asPipeOperator, PlainSelect plainSelect) {
        if ((plainSelect.getSelectItems()==null || plainSelect.getSelectItems().isEmpty()) && plainSelect.getFromItem().getAlias()==null) {
            plainSelect.getFromItem().setAlias(asPipeOperator.getAlias());
        } else {
            plainSelect = new PlainSelect().withFromItem(new ParenthesedSelect().withSelect(plainSelect).withAlias(asPipeOperator.getAlias())).addSelectItem(new AllColumns());
        }
        return plainSelect;
    }
    ...
    public PlainSelect visit(SelectPipeOperator selectPipeOperator, PlainSelect plainSelect) {
        if (selectPipeOperator.getOperatorName().equalsIgnoreCase("SELECT")) {
            if (plainSelect.getSelectItems()==null || plainSelect.getSelectItems().isEmpty()
                    || ( plainSelect.getSelectItems().size()==1 && plainSelect.getSelectItem(0).getExpression() instanceof AllColumns )) {
                plainSelect.setSelectItems(selectPipeOperator.getSelectItems());
            } else {
                plainSelect = new PlainSelect().withFromItem(new ParenthesedSelect().withSelect(plainSelect)).withSelectItems(selectPipeOperator.getSelectItems());
            }
        } else if (selectPipeOperator.getOperatorName().equalsIgnoreCase("EXTEND")) {
            if (plainSelect.getSelectItems()==null || plainSelect.getSelectItems().isEmpty()) {
                plainSelect.addSelectItems( new AllColumns()).addSelectItems(selectPipeOperator.getSelectItems());
            } else {
                plainSelect = plainSelect.addSelectItems(selectPipeOperator.getSelectItems());
            }
        }
        return plainSelect;
    }
    ...
    @Override
    public PlainSelect visit(TableSamplePipeOperator tableSamplePipeOperator, PlainSelect plainSelect) {
        return plainSelect;
    }

    @Override
    public PlainSelect visit(SetOperationPipeOperator setOperationPipeOperator, PlainSelect plainSelect) {
        return plainSelect;
    }
